...
$j(document).ready(function($j){	
	...
	$j.ajax({
		...
		success: function(data){
			...
			trace_top(graphe_top, actuel, futur, top);
			...
			slider_N_q.attachEvent("onChange",function(pos,slider){
				...
				maj_top(graphe_top, actuel, futur, top);
			});
			...
			function trace_top(graph, avant, apres, top) {
				var courbes = new Array(10*top+1);
				for (i=0; i<10*top+1; i++) { courbes[i] = { apres: apres[i+1000-10*top]/12, avant: avant[i+1000-10*top]/12 };	}
				graph.setGallery(cfx.Gallery.Lines);
				graph.setDataSource(courbes);	
			...
			function maj_top(graph, avant, apres, top) {
				var courbes = new Array(10*top+1);
				for (i=0; i<10*top+1; i++) { courbes[i] = { apres: apres[i+1000-10*top]/12, avant: avant[i+1000-10*top]/12 };	}
				graph.setDataSource(courbes);					
				graph.getSeries().getItem(1).setText("revenus mensuels actuels    (en €)");
