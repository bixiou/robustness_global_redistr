...
# gcs_split_control <- lm(reg_formula("gcs_support > 0", c("variant_split", control_variables)), data = all, weights = weight, subset = all$country != "RU")
# ics_belief_control <- lm(reg_formula("ics_support > 0", c("variant_belief", control_variables)), data = all, weights = weight, subset = all$country != "RU")
