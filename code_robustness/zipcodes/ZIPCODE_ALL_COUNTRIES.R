######################################################################################################################################################################################################################
#################################  ZIPCODE, URBANITY, REGION #####################################################################################################################################################################

'
This script processes ostal code data to assign regional and urbanity 
labels for :
Poland
Spain
Switzerland
...
Great Britain
United States
France (only the distribution of population by urbanity and region)



Postal codes are grouped into 2 to 5 macro-regions, depending on the country.
DGURBA (Degree of Urbanization) or equivalent classification is standardized as "urbanity".


The final output consists of CSV files containing zipcode, urbanity, 
and regional classifications.


The script also includes — for several countries — the calculation of the population distribution
across urbanitylevels and regional groupings, based on the most recent available demographic data.

For European countries (except France), urbanity is harmonized using official EUROSTAT statistics, based on the DGURBA classification:
https://ec.europa.eu/eurostat/databrowser/view/ilc_lvho01__custom_16633018/default/table?lang=en



Importants GITHUB Path : 
 1. https://github.com/bixiou/robustness_global_redistr/tree/main/code_robustness/zipcodes 
 2. https://github.com/bixiou/robustness_global_redistr/tree/main/data_ext/source_zipcode
 3. https://github.com/bixiou/robustness_global_redistr/tree/main/data_ext/zipcode_urbanity_region
 4. https://github.com/bixiou/oecd_climate/tree/main/code_oecd/zipcodes
...
################################################################################
# Charger les données pour  Poland, Spain, Switzerland here (taille > 100 Mo)
#source https://gisco-services.ec.europa.eu/tercet/NUTS-2024/PCODE_2024_PT.zip


# Upload data – You need to specify the path to the file you downloaded from the link above
zipcode <- st_read("../PCODE_2024_PT/PCODE_2024_PT.shp")
...
nuts <- read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/NUTS2021-NUTS2024.xlsx", sheet = 3, startRow = 1, colNames = TRUE)
...
#CSV POLOGNE
write.csv(zipcode_poland_2, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_PL_region2_teste2.csv", row.names = FALSE, quote = FALSE)

write.csv(zipcode_poland_4, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_PL_region4.csv", row.names = FALSE, quote = FALSE)



#-----------------CALCUL QUOTA 2 REGIONS POLAND----------------------------------------
 # ou readxl

# Leia o arquivo .xls
quota_reg <- read_excel("../robustness_global_redistr/data_ext/source_zipcode/tabela14.xls",  skip = 4, col_names = TRUE
)
...
# Remove all objects from the environment except "zipcode" and "nuts"
rm(list = setdiff(ls(), c("zipcode", "nuts")))
...
#CSV SPAIN
write.csv(zipcode_spain_5, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_ES_region5.csv", row.names = FALSE, quote = FALSE)

#--------------URBANITY QUOTA POP 2017 ET POP 2024 CHECK - EUROSTAT-----------------------#
#https://www.ine.es/jaxiT3/Tabla.htm?t=29005
pop <-  read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/29005.xlsx",
                                       startRow = 8, sheet = 1)
# Rename first column to 'city_raw'
names(pop)[1] <- "city_raw"

# Rename year columns to 'pop_XXXX'
names(pop) <- gsub("^X?(\\d{4})$", "pop_\\1", names(pop))

# Split city code and name
pop <- pop %>%
  separate(city_raw, into = c("city_code", "city_name"), sep = " ", extra = "merge") %>%
  filter(!is.na(pop_2024), !is.na(pop_2017))  # Keep only rows with valid population data


#-----------prepare zipcode_ES to merged

# Check if DGURBA varies within same municipality
zipcode_spain %>%
...
# Result: There is no variation in DGURBA within each NSI_CODE_2 (since filtering for Spain in the original 'zipcode' dataset kept only one observation per municipality). 
# Therefore, I will drop duplicate NSI_CODE_2 entries — although a municipality (NSI_CODE_2) may be linked to multiple postal codes, only one DGURBA value is needed per municipality 

# Simplify zipcode data by keeping first entry per city
...
# Merge population with zipcode data 
...
# Total population without DGURBA information: 
# Result — approximately 906,162 people (about 2% of the 2024 population) could not be classified due to missing DGURBA data.
merged_data %>% 
...
# Total population with valid DGURBA (98% of pop)
merged_data %>%
...
# Keep only rows with DGURBA
merged_data_valid <- merged_data %>%
  filter(!is.na(DGURBA))

# Total population with valid DGURBA
pop_total_dgurba <- sum(merged_data_valid$pop_2024, na.rm = TRUE)

# Population by DGURBA 2024 and 2017 (absolute and share)

pop_by_dgurba <- merged_data_valid %>%
  group_by(DGURBA) %>%
  summarise(
...
# Drop population columns, keep only shares
pop_by_dgurba <- pop_by_dgurba %>%
  select(-pop_2024, -pop_2017)  %>%
  st_drop_geometry()
...
# Due to the updated DGURBA classification by EUROSTAT, we relied on their published statistics to ensure consistency and comparability.
#https://ec.europa.eu/eurostat/databrowser/view/ilc_lvho01__custom_16633018/default/table?lang=en



#-------- QUOTA: Calculation of population share by region in Spain--------------------
#SOURCE POP 2021 / https://ine.es/jaxiT3/Tabla.htm?t=


pop_2021 <-  read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/2915.xlsx",
...
# 1. Create the 'lau_code' column in the main dataset (containing only the numeric part of NSI_CODE_2)
zipcode_CH  <- zipcode_CH  %>%
...
# Load the Excel file containing Swiss geographic levels
cantons <- read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/Niveaux_géographiques.xlsx",
...
# Pad commune codes with leading zeros to ensure they have 4 digits
# Example: "23" becomes "0023"
cantons <- cantons %>%
...
# - 'Numéro.de.la.commune' becomes 'lau_code'
# - 'Nom.de.la.commune' becomes 'lau_label'
# - 'Canton' remains 'canton'
cantons <- cantons %>%
  select(lau_code = Numéro.de.la.commune, lau_label =  Nom.de.la.commune , canton = Canton)

#  Merge canton information into the zipcode_CH dataset using 'lau_code' as the key
zipcode_CH  <- zipcode_CH  %>%
  left_join(cantons, by = "lau_code")

# Check how many values are missing after the merge
sum(is.na(zipcode_CH$canton))      # Number of missing canton values
sum(is.na(zipcode_CH$lau_label))   # Number of missing commune name (label) values
sum(is.na(zipcode_CH$lau_code))    # Number of missing lau_code values (should be none if data is clean)


#https://ec.europa.eu/eurostat/documents/345175/6787248/Population_data.zip

# ComplementaryLAU (Local Administrative Unit) dataset from Excel
other_lau <- read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/CH_final.xlsx")

# Select and rename only the necessary columns: commune code, name, and canton
other_lau <- other_lau %>%
  select(lau_code = LAU_CODE, lau_label = LAU_LABEL, canton = KANTON)



## Join the corrected canton info into the zipcode_CH dataset using the commune code (lau_code)
zipcode_CH <- zipcode_CH %>%
  left_join(other_lau, by = "lau_code", suffix = c("", "_new")) %>%
  mutate(
    # If the original 'canton' is missing, fill it with the value from 'canton_new'
    canton = if_else(is.na(canton), canton_new, canton)
  ) %>%
  # Remove the temporary column used during the merge
  select(-canton_new)


# Inspect which entries still have missing 'canton' info
zipcode_CH%>%
  filter(is.na(canton)) %>%
  select(POSTCODE, LAU_NAT, lau_code)


# Manually assign missing cantons based on known ZIP codes
zipcode_CH <- zipcode_CH %>%
...
# Map canton codes (e.g., "ZH") to full canton names
zipcode_CH  <- zipcode_CH  %>%
...
# Assign macro-regions based on linguistic zones
#1. Suisse alémanique, Suisse française et Suisse italienne
zipcode_CH  <- zipcode_CH  %>%
  mutate(macroregion = case_when(
    ...
    ) ~ "Suisse française", # French-speaking Switzerland
    
    canton_nome == "Ticino" ~ "Suisse italienne",  # Italian-speaking Switzerland
    
    TRUE ~ NA_character_
  ))

# Add numeric codes for macroregions
zipcode_CH  <- zipcode_CH  %>%
...
# Check for NAs generated after the join
sum(is.na(zipcode_CH$macroregion_num))



# Final cleanup: remove spatial geometry (if present) and keep relevant columns
zipcode_CH  <- zipcode_CH  %>%
...
# Save final Swiss ZIP code dataset (with urbanity and macroregion info) as CSV
write.csv(zipcode_CH , "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_CH_region_gh.csv", row.names = FALSE, quote = FALSE)
...
data2 <- read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/Classificazioni statistiche-e-dimensione-dei-comuni_31_12_2024.xlsx", 
...
# Check quotas : 2 regions (used in the 2025 survey)
...
# Recoder 2 Region : North == 1 ; South == 2 
...
write.csv(final,"../robustness_global_redistr/data_ext/zipcode_urbanity_region/IT_zipcode_pop_2023_region4.csv", row.names=F) #.cvs with 4 regions
write.csv(final_2,"../robustness_global_redistr/data_ext/zipcode_urbanity_region/IT_zipcode_pop_2023_region2.csv", row.names=F) # .csv with 2 regions (used in the 2025 survey)
...
data1 <- read.csv("../robustness_global_redistr/data_ext/source_zipcode/source_JA_zipcode.csv", sep = ",", fileEncoding = "Shift-JIS", header = F) # Zipcode 
data.pop2 <- read.csv2("../robustness_global_redistr/data_ext/source_zipcode/source_JA_population.csv", sep=",", fileEncoding = "cp932", header = F, skip = 9) #Population by Municipality in 2020
...
write.csv(merge_final2,"../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_JA.csv", row.names=F, quote = FALSE)
...
'
METHODO : URBANITY JAPAN
https://fr.wikipedia.org/wiki/District_(Japon)
http://www.tt.rim.or.jp/~ishato/tiri/code/code.htm
https://fr.wikipedia.org/wiki/Syst%C3%A8me_d%27adressage_japonais#:~:text=Depuis%20la%20r%C3%A9forme%20de%201998,agit%20d%27un%20code%20postal.

https://donnees.banquemondiale.org/indicateur/SP.URB.TOTL.IN.ZS

Urban if the municipality type is a “shi” or a “ku”
Rural if the municipality type is a "cho", "machi", "mura" or "son"



REGION (R code)

North : "HOKKAIDO", "AKITA KEN", "AOMORI KEN", 
                                     "FUKUSHIMA KEN", "IWATE KEN", "MIYAGI KEN",
                                     "YAMAGATA KEN"

South : "HIROSHIMA KEN", "OKAYAMA KEN", "SHIMANE KEN", 
                                     "TOTTORI KEN", "YAMAGUCHI KEN", "EHIME KEN", 
                                     "KAGAWA KEN", "KOCHI KEN", "TOKUSHIMA KEN", 
                                     "FUKUOKA KEN", "KAGOSHIMA KEN", "KUMAMOTO KEN",
                                     "MIYAZAKI KEN", "NAGASAKI KEN", "OITA KEN", 
                                     "OKINAWA KEN", "SAGA KEN"

Kanto : CHIBA KEN", "GUMMA KEN", "IBARAKI KEN", 
                                     "KANAGAWA KEN", "SAITAMA KEN", "TOCHIGI KEN",
                                     "TOKYO TO"

Kansai : "HYOGO KEN", "KYOTO FU", "MIE KEN", "NARA KEN",
                                     "OSAKA FU", "SHIGA KEN", "WAKAYAMA KEN"

Chubu : "AICHI KEN", "FUKUI KEN", "GIFU KEN",
                                     "ISHIKAWA KEN", "NAGANO KEN", "NIIGATA KEN",
                                     "SHIZUOKA KEN", "TOYAM
...
total_by_region <- total_by_region %>%
  mutate(region_share = region_total / sum(region_total))
...
write.csv(result_extended, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_DE_complete.csv", row.names = FALSE, quote = FALSE)
...
1 -Eastern
2 -Southern
3 -Others [Northern, Western, Central]

Northern <- c("01", "02", "03", "04", "13")  # Schleswig-Holstein, Hamburg, Niedersachsen, Bremen
Western <- c("05", "07", "10")  # Nordrhein-Westfalen, Rheinland-Pfalz, Saarland
Central <- c("06", "16")  # Hessen, Thüringen
Eastern <- c("11", "12", "14", "15")  # Berlin, Brandenburg, Sachsen, Sachsen-Anhalt, , Mecklenburg-Vorpommern
Southern <- c("08", "09")  # Baden-Württemberg, Bayern
...
data <- read.csv2("../postcodes.csv", sep=',')
...
write.csv(final,"../robustness_global_redistr/data_ext/zipcode_urbanity_region/UK_zipcode.csv", row.names=F, quote = FALSE)
...
write.csv(merge_1, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/us_25_zipcode.csv", row.names = FALSE, quote = FALSE)
...
write.csv(final_data, "../robustness_global_redistr/data_ext/zipcode_urbanity_region/zipcode_US_region.csv", row.names = FALSE, quote = FALSE)
...
Description - based on geographical distributions of zipcodes
...
data <- read.xlsx("../robustness_global_redistr/data_ext/source_zipcode/estim-pop-dep-sexe-gca-1975-2025.xlsx",
  sheet = 2,
  startRow = 5
)[1:105, 1:8]
...
'
METHODO :  zipcode France Methode


# 1 - Ile de France 
# 2 - Hauts de France, Grand-Est, Bourgogne, Franche-Comté
# 3 - Sud-Ouest - "09", "12", "19", "23", "24", "31", "32", "33", "40", "46", "47", "48", "64", "65", "81", "82", "87" [Nouvelle Aquitaine et une partie de occitanie]
# 4 - Sud-Est - "01", "03", "07", "11", "13", "15", "26", "30", "34", "38", "42", "43", "63", "66", "69", "73", "74", "83", "84" [PACA, Auvergne Rhône-Alpes et une partie de occitanie]
# 5 - Reste [Bretagne, Normandie, Centre-Val de Loire, Outre mer]
'
