...
package("openxlsx2") # wb_load, open-ended fields
...
#' package("processx")
package("readstata13") # read.dta13
...
is.pnr <- function(variable, empty_as_pnr = T) { # barres
  if (empty_as_pnr) {
...
quadratic_interpolation <- function(x0, x1, y0, y1, integral, linear_if_non_monotone = T) { # Returns a quadratic function f such that int(f)_x0^x1 = integral, f(x0)=y0, f(x1)=y1
  matrix_A <- matrix(c( # Set up the matrix for the system of equations
    1, x0, x0^2,        # Equation for f(x0) = y0
    1, x1, x1^2,        # Equation for f(x1) = y1
    x1 - x0, (x1^2 - x0^2) / 2, (x1^3 - x0^3) / 3 # Equation for integral
  ), nrow=3, byrow=TRUE)
  vec_b <- c(y0, y1, integral) # Vector representing the right-hand side of the equations
  coeffs <- solve(matrix_A, vec_b) # Solve the system to get the coefficients
  f <- function(x) {  coeffs[1] + coeffs[2] * x + coeffs[3] * x^2 }
  df <- function(x) {  coeffs[2] + coeffs[3] * 2 * x }
  if (linear_if_non_monotone & (df(x0) * df(x1) < 0)) { # If the quadratic interpolation yields a non-monotone function, use a linear interpolation (that violates the integral) instead
    coeffs <- solve(matrix(c(1, x0, 1, x1), nrow=2, byrow=TRUE), c(y0, y1))
    f <- function(x) {  coeffs[1] + coeffs[2] * x } # to preserve monotonicity, integral and smoothness, interpolate cubic spline that maximizes the product of derivatives at the knot ?
  }
  return(f)
}
quadratic_interpolations <- function(averages, thresholds, x_coarse, x_fine) { # Interpolates piecewise at points x_fine between points in x_coarse such that f(x_coarse) = thresholds and integrals = averages
  if (any(sort(averages) != averages) || any(sort(thresholds) != thresholds) || any(sort(x_coarse) != x_coarse) || any(sort(x_fine) != x_fine)) warning("Non-increasing inputs in quadratic_interpolations")
  if (length(thresholds) == length(averages)) thresholds <- c(thresholds, averages[length(thresholds)])
  y_fine <- c()
  j <- 1
  for (i in 2:length(x_coarse)) {
    f <- quadratic_interpolation(x_coarse[i-1], x_coarse[i], thresholds[i-1], thresholds[i], averages[i-1])
    while (j <= length(x_fine) && x_fine[j] <= x_coarse[i]) {
      y_fine <- c(y_fine, f(x_fine[j]))
      j <- j+1
    }
  }
  return(y_fine)
...
#   return(levels)
# }
Levels <- function(variable, data = e, miss = TRUE, numbers = FALSE, values = TRUE, concatenate = FALSE, max_values = 13, names = FALSE, logT = F) {
...
mean_ci_along_regressions <- function(regs, along, labels, df = e, origin = 'others_at_mean', logit = c(FALSE), logit_margin = T, confidence = 0.95, factor_along = FALSE, covariates = NULL, weight_non_na = T,
  ...
  # Note that others_at_mean is not the same as unconditional mean by group since others_at_mean disregards correlations between along and the other variables.
  # TODO: logit, origin
  ...
  if (!is.null(subsamples) && subsamples == along && !is.null(covariates)) {
    ...
    if (length(names_levels) == 1 & any(grepl("(", names_levels, fixed = T))) {
    ...
    for (r in regs) {
      ...
      if (origin == 'intercept') origin_value <- reg$coefficients[["(Intercept)"]]
        ...
        origin_value <- wtd.mean(dependent_var[data_s[[along]] %in% levels_along[[1]]], weights = data_s$weight[data_s[[along]] %in% levels_along[[1]]])
      } else origin_value <- 0
      if (logit[i] & logit_margin) { # logit margins
        # if ("lm" %in% class(reg)) warning("Logit margins should be provided: logitmfx(..)")
        for (l in names_levels[min(2,k):k]) if (!l %in% row.names(regmxf)) {
        ...
        coefs <- origin_value + c(0, regmxf[names_levels[min(2,k):k],1])
        ...
        SDs <- sapply(levels_along, function(i) return(sqrt(wtd.mean(((data_s[[along]] %in% i) - wtd.mean(data_s[[along]] %in% i, weights = data_s$weight, na.rm = T))^2, weights = data_s$weight, na.rm = T))))
        ...
        #   mean_ci_origin <- binconf(x = sum(data_s[[weight]][data_s[[along]] %in% levels_along[[1]]], na.rm=T), n = sum(data_s[[weight]], na.rm=T), alpha = 1-confidence) # WRONG! This is the CI for the group mean, not the CI for the regression coefficient!
        ...
        SD <- sqrt(wtd.mean(((data_s[[along]] %in% levels_along[[1]]) - wtd.mean(data_s[[along]] %in% levels_along[[1]], weights = data_s$weight, na.rm = T))^2, weights = data_s$weight, na.rm = T))
        ...
        coefs <- origin_value + c(0, reg$coefficients[names_levels[min(2,k):k]]) # what about emmeans(reg, ~ 1 | along) to compute e.g. CI_origin?
        ...
        # print(robust_SEs)
        # print(names_levels)
        for (l in names_levels[min(2,k):k]) if (!l %in% row.names(robust_SEs)) {
          warning(paste("Covariate", l, "is absent from regression, replaced by NA."))
        ...
        CI <- origin_value + rbind(CI_origin, cbind(robust_SEs[names_levels[min(2,k):k], 1] - t*robust_SEs[names_levels[min(2,k):k], 2], robust_SEs[names_levels[min(2,k):k], 1] + t*robust_SEs[names_levels[min(2,k):k], 2]))
      }
      mean_ci_reg <- data.frame(y = label, mean = coefs, CI_low = CI[,1], CI_high = CI[,2], along = names_levels) # levels_along
      mean_ci <- rbind(mean_ci, mean_ci_reg[2:max(2,k),]) # remove (Intercept) row (06/2025, dunno why it was still there)
    }
...
plot_along <- function(along, mean_ci = NULL, vars = outcomes, outcomes = paste0(vars, conditions), covariates = NULL, subsamples = NULL, conditions = c(""),
  ...
  plot <- ggplot(mean_ci) + lapply(origins, function(xint) geom_vline(xintercept = xint, linetype = "longdash", color = "grey50")) + # For plot, we need mean_ci (cols: mean, CI_low,high, variable, along), legend_x, legend_y. For save, we need: name, folder, width, height.
    ...
    theme(base_size = font_size, panel.grid.major.y = if (length(unique(mean_ci$along)) > 1) element_blank(), panel.grid.minor.y = if (length(unique(mean_ci$along)) > 1) element_blank(), text = element_text(color = "black"), axis.text.y.left = if (no_label) element_blank() else ggtext::element_markdown(), axis.text.x.bottom = ggtext::element_markdown(color = "black"), axis.title.y = ggtext::element_markdown(color = "black"), axis.title.x = ggtext::element_markdown(color = "black"), axis.text = ggtext::element_markdown(color = "black"), legend.text = ggtext::element_markdown(color = "black"),
        legend.title = element_blank(), legend.position = ifelse(no_legend, "none", ifelse(legend_top, "top", "right")), legend.background = element_rect(color = if (legend_box) "black" else "white")) + #legend.title = element_text(color = "black"),
