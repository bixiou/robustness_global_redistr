...
I verify that the results are similar when using a conditional logit: the average marginal effects of tax and aid are 6~p.p. and $-$3~p.p., respectively.}
...
Comparing absolute values of coefficients from different regressions is a statistical challenge. As an approximation, I regress the absolute effect size of any policy on a constant and a dummy for either the tax or aid policy, weighting by countries' population sizes. The tax effect is not significantly different from the average absolute effect size (just 4\% lower, $p$=.91) but the aid effect is significantly lower ($p$=.07).
