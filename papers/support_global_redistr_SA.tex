...
% Order: EJ (no word limit, 100£), JEEA (200$), AEJ:EP (no word limit, 700$), AJPS (10k free, everything incl. reference except abstract), BJPS (10k free, main body + figures + footnotes), WD (free, 10k main body), JoP (35p double space, free), Ecol Eco (8k, free), Science Advances (15k, 5k$)
% 11600 words + 2300 outside text
% TODO: check AER, revision
% TODO: add field, etc. figures on main sociodemos rather than countries (poor/rich; education levels; vote; age...)
% TODO fix bug program_preferred_by_cut_aid_in_program_pol
% TODO cite Drupp et al 25 revision
% TODO? no: cite Noel & Thérien: 80\% are in favor of international redistribution in 10 European countries in 95
% TODO revision cite Smith et al. 25
% TODO? reparations cite Ogunbode?
% TODO past or present when describing the survey?
% TODO!? q-values (multiple hypotheses testing); multi-level modeling (explaining country-level support); Gini before/after income redistr; Dem/swing states (cf. pol_US figs)
...
% https://x.com/adrien_fabre/status/1931090260663861546
% _______
...
% The 2% tax on billionaires' wealth proposed by @gabriel_zucman receives the strongest support, with a majority of (absolute) support in every country. 
% ________
...
% - 5 p.p. more likely to be preferred if it contains a globally redistributive wealth tax
% - 3 p.p. less likely to be preferred if it contains cutting development aid.
% ______
% Direct questions confirm this finding:
...
% _______
% Over two-thirds of people favor a sustainable future with half as much flying and beef consumption and without thermal cars, over the status quo with their lifestyle maintained.
% ________
% Support for an international policy decreases only slightly as country coverage shrinks. 
...
% ______
% We also see that for a Climate Scheme: 69% support if global if participating countries cover 72% of world emissions, still 65% support if it only covers 33% of emissions.
% _____
...
% To know more about the GCS, see video: bit.ly/CH_gcp or book: bit.ly/bookGCP
% _____
...
% _______
% To sum up, most people in high-income countries would accept global redistribution policies. 
...
% You can do it too here: https://www.centre-cired.fr/en/custom-global-redistr/
...
% Such a redistribution would be a game-changer for the global poor.
...
\usepackage[numbers,sort&compress]{natbib} % Science Advances: numbered references cited in order of appearance
\usepackage{bibunits} % To get multiple bibliography (MUST be loaded AFTER natbib, else numbered citations hang at the bibliography)
\usepackage{url} % [hyphens]
% Science Advances: backref disabled. With numbered (numbers-mode) citations,
% backref/pagebackref loops while typesetting the bibliography's .brf back-references,
% and journal reference lists should not carry "cited on page N" annotations anyway.
% \usepackage[hyperpageref]{backref} % back references biblio. Needs latexmk at compilation.
% \usepackage[pagebackref]{hyperref}
\usepackage{hyperref}
\usepackage{etoolbox}
% % Lines below required to make pagebackref compatible with bibunits (only needed with backref)
% \makeatletter
...
\setcitestyle{round} % Science Advances: parenthetical numbered citations (numbers & sort&compress set via the natbib package options above)
...
% \renewcommand{\thesection}{\Roman{section}} % AER
...
% \defaultbibliographystyle{aea} %  AER
\defaultbibliographystyle{unsrtnat} % Science Advances: numbered, in order of citation
% \defaultbibliographystyle{chicago} % APSR
...
% TODO!?
% Main contrib: Global redistribution enjoys stable majority acceptance across all high-income countries, and this is robust to realistic concerns (coverage, warm glow, vote choice). 
% This paper shows that support for global redistribution is systematically underestimated, structurally under-salient, yet robustly majoritarian and electorally consequential. This combination of high acceptance, low salience, and strong pluralistic ignorance generates a coordination failure that may explain the political marginalization of global redistribution.

% Main conceptual framework: Why might support be understated in political discourse? (pluralistic ignorance, low salience, coordination failure) [Alternative: Why might people support it? (universalism? reciprocity? fairness? self-interest?)]
% Clarify: concern => addressed; for each in turn: Stated preference vs revealed preference, Hypothetical nature of global policies, Online survey limitations, Possibility of expressive responding and social desirability
% Highlight conjoint results more/earlier
% open-ended fields, revenue split, custom redistribution task, extensive descriptive heterogeneity should go to an appendix or be summarized briefly
% Couldn't the treatment also increase salience or signal elite support? The exclusion restriction seems questionable 
% Alternative explanation for status quo bias (instead of warm glow): bandwagon effect (updating on social norms), noise, credibility effect (belief updating on policy features).
% Comment acceptance among far-right voters
% Engage more with the political economy literature on why democracies don't redistribute more domestically (Fastenrath, etc.). Does that logic apply globally?
% More modest causal claims. 
% preference-policy gap
% heterogeneity in anti warm glow effect => no significant effect, though effect stronger for Center and weaker for Left
...
% \footnote{I received funding from the Agence Nationale de la Recherche for this research (ANR-24-CE03-7110). %The randomized controlled trials were registered in the American Economic Association's registry for randomized controlled trials under ID \href{https://www.socialscienceregistry.org/trials/17248}{AEARCTR-0017248}. %This RCT was registered as \href{https://www.socialscienceregistry.org/trials/17248}{AEARCTR-0017248}. 
...
% preregistered in the Open Science Foundation registry (\href{https://osf.io/7mzn4}{osf.io/7mzn4}). 
% I am grateful to Alina Timoshkina and Gerhard Toews for helping me launch the survey in Russia. I thank researchers who kindly gave me access to their data: Félix Bajard for wealth distribution and Amory Gethin for income distribution. %, Gerhard Toews for Russian census. 
% I am grateful to Armon Rezai and Wien Universität for helping me use Qualtrics. I thank Romain Espinosa for his attentive reading and useful feedback. I am grateful to the translators and the numerous people who graciously tested the survey, proofread the translations, and gave feedback, including Gianluca Drappo, Valeria Glebova, Anne Guillemot, Marya Kayyal, Lu Li, Lilian Leupold, Guadalupe Manzo, Rintaro Matsuda. % Pauline? Samuel? Davide Pace? Mari Mohamad?
...
\author{[Authors removed for double-blind review]\footnote{[Author affiliations and contact information removed for double-blind review.] 
% \\ I received funding from the Agence Nationale de la Recherche for this research (ANR-24-CE03-7110). The project has been approved by the CIRED institutional review board (IRB-CIRED-2025-2) and preregistered in the Open Science Foundation registry (\href{https://osf.io/7mzn4}{osf.io/7mzn4}). The randomized controlled trials were registered in the American Economic Association's registry for randomized controlled trials under ID \href{https://www.socialscienceregistry.org/trials/17248}{AEARCTR-0017248}. % AER 
% I am grateful to Alina Timoshkina and Gerhard Toews for helping me launch the survey in Russia. I thank researchers who kindly gave me access to their data: Félix Bajard for wealth distribution and Amory Gethin for income distribution. %, Gerhard Toews for Russian census. 
% I am grateful to Armon Rezai and Wien Universität for helping me use Qualtrics. I thank Romain Espinosa for his attentive reading and useful feedback. I am grateful to the translators and the numerous people who graciously tested the survey, proofread the translations, and gave feedback, including Gianluca Drappo, Valeria Glebova, Anne Guillemot, Marya Kayyal, Lu Li, Lilian Leupold, Guadalupe Manzo, Rintaro Matsuda. % Pauline? Samuel? Davide Pace? Mari Mohamad?
% I am grateful to Erwan Akrour, Marius Le Hénaff, and Raquel Oliveira %and Alina Timoshkina 
% for research assistance on the quotas and the pools of policies. I thank seminar, workshop or conference participants for useful feedback, at: the University of Brasília, Unicamp, the University of São Paulo, WZB Berlin, Paris-Saclay, the World Inequality Lab, MSE-Sorbonne, SKEMA, In-sted, the Bundesbank, FSR Climate Conference, the New Economic School, CIRED, Fürth. 
}
...
% \begin{center}
% {\textbf{\href{
%   %https://github.com/bixiou/robustness_global_redistr/raw/main/papers/aer.pdf}{Link to clickable 
% https://github.com/bixiou/robustness_global_redistr/raw/main/papers/support_global_redistr.pdf}{Link to most recent 
% version}}} % AER 
% \end{center}
...
% À partir d'une enquête originale menée auprès de 12 000 répondants représentatifs de onze pays à hauts revenus (les États-Unis, le Japon, la Russie, l'Arabie saoudite et sept pays européens), j'examine le soutien du public aux politiques mondiales de redistribution et de lutte contre le changement climatique, ainsi que sa sensibilité à des caractéristiques politiques clés telles que l'ampleur des transferts et la couverture géographique. Bien que les inégalités mondiales ne constituent pas une préoccupation majeure, elles sont perçues comme une injustice importante. Dans chaque pays, la majorité des personnes interrogées acceptent la quasi-totalité des politiques mondiales testées, y compris celles qui redistribueraient 5 % du revenu mondial ou entraîneraient des coûts  monétaires pour les personnes interrogées. Un traitement informationnel montre que le soutien aux politiques mondiales augmente de manière causale parmi les personnes interrogées qui les perçoivent comme probables, un effet opposé à celui du « warm glow ». Le soutien aux politiques internationales ne diminue que légèrement lorsque la couverture géographique diminue. Dans l'ensemble, les résultats renforcent les conclusions des enquêtes précédentes et suggèrent qu'une large coalition de pays pourrait faire progresser le développement soutenable. 

% Acceptance increases following a treatment that boosts the belief that global redistribution is likely. 

% AI: I am a researcher and am drafting an academic paper intended for publication in the American Economic Review. I want you to help me improve the English style of that paper, paragraph by paragraph. Keep your responses concise: no need to comment on your editing suggestions, just give the suggested edited paragraph. Highlight (e.g. in bold or strikes) the changes you make. 
% I want to stress that global policies would be *accepted* rather than supported, in the sense that the public is unlikely to mobilize against such policies should they be implemented, but is also unlikely to actively demand them. Keep that in mind while editing (i.e. prefer the use of "acceptance" over "support").
...
% Science Advances requires a one-sentence summary (Teaser) of the work.
\noindent\textbf{One-sentence summary:} Majorities in high-income countries accept international redistribution and climate policies, even when entail costs.
 % robustly to their cost, country coverage, and the prospect of implementation.
...
% \textbf{Keywords:} Redistribution, global policies, cap-and-trade, attitudes, survey, warm glow.%, inequality, wealth tax. % NCCcomment

\clearpage
% \tableofcontents % AER
% \addtocontents{toc}{\protect\setcounter{tocdepth}{-2}} % To remove main text from ToC
...
} % more than enough to finance the eradication of extreme poverty \citep{sahoo_what_2025}. % AER revision keep end?
...
Despite strong stated acceptance even in HICs, international redistribution remains limited. 
This preference--prominence puzzle could be explained by an overstatement of support in previous surveys or by a discrepancy between public attitudes and official policy. In this paper, I solve the puzzle by demonstrating how low salience and pluralistic ignorance can reconcile strong acceptance with low prominence. 

 % rarely discussed in public debates, let alone advocated % championed? embraced % 
...
% % and another hypothesis is needed to 
% but could not 
% explain the lack of prominence of this issue. 
%%%
% A promising candidate is ``pluralistic ignorance'': the underestimation of public support. Indeed, pluralistic ignorance has been documented regarding %costly 
...
% Main cites: Vlasceanu, -Ghassim, -Cappelen, Dannenberg 2010 (negotiators' equity pref), -Boneva (both), -Gustafson, -Enke, -Fehr, Carattini, -Mildenberger, -Dechezlepretre, (Meilland, Beiser-McGrath 19,) Young-Brun, MacKay, -Bechtel & Scheve, -Toews 
...
% In this paper, 
I conduct a pre-registered large-scale survey to examine whether global redistribution policies are robustly accepted and to investigate the reasons for their lack of prominence. I test several hypotheses. Surveying eleven HICs, I test whether international policies are accepted by majorities in countries considered conservative and not yet surveyed on this topic, such as Japan, Poland, Russia, Saudi Arabia, and Switzerland. Recognizing that some key countries would likely not participate if these policies were implemented, I test how acceptance is affected when a climate scheme or a wealth tax is international but not truly global. I explore three other % potential 
mechanisms that could explain the  preference--prominence gap. 
First, I test for pluralistic ignorance. Second, I analyze the salience of global redistribution, and whether it is a vote-determining issue. Third, I test for ``warm glow'', whereby people delude themselves into supporting hypothetical policies in order to ease their conscience, notably by testing whether the support is only claimed for as long as the policies are deemed unlikely. Finally, I explore a variety of international policies, ranging from the plausible to the radical. % TODO! rewrite to problematize, see e.g. grant proposal
% Hypotheses: warm glow, salience, pluralistic ignorance, non-global or new countries
% The public support for internationally redistributive policies observed in previous surveys might have been overstated according to some hypotheses. A first hypothesis is that a large part of the population supports a policy of global redistribution for as long as its implementation seems unlikely, in order to ease their conscience. If support were mainly due to such a psychological mechanism, called warm glow, it might dissipate when the prospect of the policy materializes (H1a) or if the policy can be replaced a less costly substitute with the same moral appeal (H1b). Second, although previous surveys detailed the direct costs and opportunity costs of globally redistributive policies for the respondent and their fellow citizens, the salience of global issues in the surveys may have created a context favorable to universalistic answers (H2). Second, while previous surveys have tested truly global policies, the support may be lower in the likely case where some countries do not participate (H3). To assess the robustness of the strong support for a cap and share and other globally redistributive policies found in previous surveys, I

% revision: put back?
% Throughout the paper, I make a distinction between \textit{support} and \textit{acceptance}. I use the term \textit{support} to refer to the absolute share of \textit{Somewhat} or \textit{Strong support} on Likert scales, and \textit{acceptance} to refer to relative support ---specifically, the support share among non-\textit{Indifferent} responses.\footnote{Although binary (\textit{Yes}/\textit{No}) questions are typically worded in terms of ``support'', I conservatively report their results using the term \textit{acceptance}. % TODO? for Yes/No items, “acceptance” is simply the proportion answering “Yes,” and that the change in term is intended to signal the possibility of weaker acquiescence in a forced-choice format rather than a different estimator
% This approach avoids mistaking passive consent for active support among respondents in questions for which they could not choose a neutral option.}
...
Radical proposals are also widely accepted. Majorities in every country agree that ``governments should actively cooperate to have all countries converge in terms of GDP per capita by the end of the century'', and that globally coordinated climate policies are preferable to the status quo, even if they entail completely electrifying cars by 2045 and doubling the prices of heating fuel, flights, and beef. Overall, I find 64\% acceptance for a progressive income tax that would finance poverty reduction in the Global South, which would collect 5\% of world income from the global top~3\%, with marginal tax rates ranging from 15\% above \$80,000 per year to 45\% above \$1 million. Relatedly, in an interactive task where respondents design their preferred global income redistribution, nearly half choose a redistribution that would make them poorer (versus less than 10\% choosing one that would make them richer). The average custom redistribution entails over 5\% of world income in transfers from the rich to the poor. 

Although stated answers seem to suggest a strong acceptance of international redistribution policies, the strength and consistency of this support can only be confirmed in situations where respondents are not primed to focus on this issue, and where international redistribution is competing with other desirable goals. To achieve this, I test the prioritization of this topic using three experiments at the start of the survey. 
Before respondents could infer the survey's topic, they had to complete an open-ended field, a conjoint experiment, and a budget allocation task. 

When asked to allocate the revenue from a %hypothetical 
...
While policies to address global inequality are widely accepted, they have low salience. Indeed, this topic is rarely mentioned in open-ended fields at the beginning of the survey, where respondents were asked to write about various considerations. Respondents' top concern is the cost of living, and their most frequent wish is for greater purchasing power. While inequality is most often regarded as the greatest injustice ---with some inconclusive indications that these responses relate to inequality at the global level--- global inequality almost never appears among issues respondents consider important but neglected in public debate. The low priority placed on global redistribution may explain why it is seldom discussed in public debates, despite widespread acceptance of related policies.

Despite its low salience, global redistribution may be a vote-determining issue for some people, as %indicated by % revision
the conjoint experiment suggests. 
In this task, respondents express their preference between two political programs, each composed of policies randomly selected from those prominently debated in their country. When a program includes a \textit{global tax on millionaires with 30\% of the revenue funding LICs}, the likelihood of that program being preferred increases by 5~p.p., while \textit{cutting development aid} reduces it by 3~p.p.  A direct question confirms that some voters might change their vote intention if a candidate campaigned on sustainable development: 36\% of respondents report they would be more likely (versus 17\% less likely) to vote for a party if it participated in a global movement for climate action, taxes on millionaires, and poverty reduction in LICs. In a related question, 68\% of respondents (and 52\% of the 561 millionaires\footnote{Millionaire is understood as having a net wealth of at least one million dollar or the equivalent in other currencies, cf. Question~\ref{q:millionaire}.} % revision
who responded) state they could actively participate in such a movement (either by signing a petition, attending a demonstration, going on strike, or donating to a strike fund). 

Although low salience contributes to the lack of prominence of international redistribution, it cannot fully explain why related proposals are so rarely put forward by non-profit organizations or electoral candidates, especially given the apparent electoral potential. I test two hypotheses that could help explain this puzzle: pluralistic ignorance and unreliable support. I find evidence only for the former.

I identify pluralistic ignorance through an incentivized question that asks respondents for their belief regarding support for the Global (version of the) Climate Scheme (GCS), either among their compatriots or in the U.S. In Japan and in European countries, there is majority support for the GCS, yet most people believe there is not. Overall, the median respondent underestimates support in their own country by 16~p.p. and non-American respondents underestimate support in the U.S. by 22~p.p. Pluralistic ignorance may be an important reason why global solidarity solutions are neglected.
...
In other words, I find no evidence of \textit{warm glow}. On the contrary, the effect goes in the opposite direction compared to the \textit{warm glow} hypothesis: if people believed that a global policy were likely, they would be more likely to support it (which can be interpreted as %there is suggestive evidence of %
...
Universalism has been identified as one of the best predictors of voting behavior \citep{enke_moral_2020} and ideology \citep{enke_moral_2023,cappelen_universalism_2025}, particularly in %though the strong correlation is limited to 
Western countries \citep{cappelen_universalism_2025}. 
I use a new question to measure universalism, asking respondents which group they advocate for when they vote. 45\% choose a universalist response (either ``Humans'' or ``Sentient beings (humans and animals)''), while 32\% opt for their fellow citizens. Using a variance decomposition, I find that universalism is a stronger predictor of policy attitudes than sociodemographic variables such as income, country, or even vote choice, echoing the results of \cite{enke_moral_2023}. % For Enke, it's a better predictor than vote only for the amount allocated to foreign aid (in a budget allocation task)
...
Besides, there is a majority of universalists in Europe, Saudi Arabia, and among left-wing voters. 
...
There are few cross-country surveys on international policies. They consistently find strong public support for globally redistributive policies \citep{cappelen_majority_2025,fabre_majority_2025} or global democratic governance \citep{ghassim_who_2024}. % re-cite universalism?

% Beyond papers closest to it, this paper contributes to other strands of literature. In political science, issue salience has 

The first questions on respondents' considerations contribute to an extensive literature in political science on ``issue salience'' ---the priority attributed to a given issue. Issue salience is now widely acknowledged as a key factor in determining voting behavior \citep{repass_issue_1971,egan_partisan_2013,krosnick_role_1988,dennison_review_2019,edwards_explaining_1995}. Furthermore, according to open-ended responses, the ``most important issues'' relate to the economy and healthcare \citep{wlezien_salience_2005}. Although climate change and hunger appear in the top five problems when the question is framed at the global rather than national level \citep{yeager_measuring_2011}, public acceptance of sustainable development policies may be overshadowed by more pressing concerns in voters' choices. Meanwhile, 40\% of Norwegian respondents cite the international distribution of costs in an open-ended question about climate change injustice, by far the most cited category \citep{gregersen_poor_2025}. % revision cite Fastenrath & Marx
% For example, \cite{dennison_how_2020} explains the rise of the far right in Europe with increased issue salience of immigration, while attitudes on immigration have remained stable or even improved in the last 15 years. 
% Similarly, the support for global redistribution might be outshined by more prioritized concerns.

The finding of pluralistic ignorance echoes similar findings documented regarding %costly 
climate action \citep{andre_misperceived_2024,andre_globally_2024,mildenberger_beliefs_2019}, the billionaire tax \citep{cappelen_majority_2025}, and the GCS \citep{fabre_majority_2025}. 
...
\cite{nair_misperceptions_2018} finds that Americans underestimate their rank in the global income distribution by 27 percentiles on average and overestimate the global median income by a factor of 10, which lowers their support for foreign aid. Similarly, \cite{fehr_your_2022} find that 9 out of 10 Germans express support for global redistribution, even though respondents underestimate their position in the global income distribution by an average of 15 percentiles. %, although they do not misperceive their rank in the national distribution on average. 

Finally, the paper contributes to the literature analyzing dispositions towards free-riding on climate action and the ways in which support for climate agreements depends on their country coverage. 
Using conjoint analyses in Western countries, 
% \cite{bechtel_mass_2013} and % AER revision
\cite{beiser-mcgrath_could_2019} demonstrate that climate agreements with broader country coverage are %15 p.p. 
more likely to be preferred. In Germany and the U.S., \cite{gampfer_obtaining_2014} also find stronger support for funding climate action in low-income countries when the cost is shared with other countries. Nevertheless, surveys consistently show that people support their country taking unilateral climate action, even in the absence of such action in other countries \citep{bernauer_how_2015,mcgrath_how_2017,beiser-mcgrath_commitment_2019%,sivonen_attitudes_2022
}. \cite{aklin_prisoners_2020} show that the empirical evidence for free-riding is not compelling, and that climate inaction can be equally well explained by distributive conflicts. Still, survey evidence indicates some degree of conditional cooperation: support for domestic climate action increases if other countries join forces \citep{carlsson_importance_2025,tam_they_2025}. 
...
% [Materials and Methods (former `Data and Design'') moved to the end of the main text for the Science Advances initial submission.]

\section{Results}\label{sec:results}

\subsection{Salience and Prioritization of Global Solidarity\label{sec:salience}}
...
\subsubsection{Top-of-mind Considerations}\label{subsec:considerations}
...
Then, I used AI and my own reading of a few hundred answers to identify the most common concepts, from which I selected 27 categories. Next, I classified each answer into one or more of these categories, both manually (Figures \ref{fig:field_manual}-\ref{fig:injustice_field}) and automatically using AI (Figure \ref{fig:field_gpt}). Finally, I manually defined a list of 47 (disjunctions of) keywords and used it to automatically classify all responses.\footnote{The list of keywords is provided in Appendix \ref{subsec:keywords}.} Figure \ref{fig:field_keyword} reports the 24 most common keyword matches. 
...
Though the predominance of materialistic considerations is consistent with previous studies \citep{wlezien_salience_2005,singer_who_2011}, further research is needed to determine whether this arises from the context (an impersonal survey) or truly reflects people's primary thoughts. 


% # 3. keywords in fields (taken jointly) --- moved to Appendix for Science Advances initial submission (see app.tex, label fig:wordcloud)
...
Reading and coding each field not only results in an arguably more accurate classification, but also offers insight into how people think. 
For example, most people reason from their own perspective (e.g. ``my pension is too low'', ``I want to buy a house'') and do not refer to the broader picture or to political reforms. %, I report a list of topics 
% that I find overrepresented in each country
% my impressions of each country's slant. 
To get a sense of respondents' own words, random selections of responses are displayed at [link withheld for double-blind review]. %  also https://www.centre-cired.fr/random_fields/
In addition to the manual classification presented in this section, I compiled a list of topics that are uniquely prevalent in each country, which I report in Appendix~\ref{subsec:slant}. 
...
% in the U.S.: the economy, Donald Trump, breaches of the Constitution, abortion, and gun control. % TODO? put back?
...
\subsubsection{Prioritization of Public Spending Items}\label{subsec:revenue_split}
...
%; and, on average, respondents prefer to allocate 33.4\% of the revenue to LICs (versus domestic healthcare and education). 
...
% low-income countries} versus \textit{domestic} healthcare and education. The ``binary'' variant shows that around half of Americans and Western Europeans prefer to allocate half (rather than none) of the revenue to low-income countries. The ``continuous'' variant shows that the median respondent prefers to allocate 30\% of the revenue to low-income countries (LICs).
% In other words, while most people would prefer a globally redistributive tax to the status quo, the more leeway they are granted to allocate the revenue from such a tax, the less they would allocate to LICs. 
The greatest leeway tested by \cite{fabre_majority_2025} let the respondents select their preferred share for LICs versus domestic healthcare and education, %one alternative spending item, 
and the average preference was 33.4\% ---that is, 66.8\% of an equal split. % \footnote{The greatest leeway possible would be an open-ended field, but its interpretation would be challenging as answers that do not specify the geographical scope would be equivocal.}
Naturally, one expects respondents to split the revenue among all desirable spending items, so %that the revenue allocated to one item depends on the number of items. Therefore, 
if LICs compete with not one but several national items, the share allocated to LICs is expected to diminish. 
If this share is less than 67\% of an equal split, it would mean that \cite{fabre_majority_2025} overestimated the prioritization of LICs, perhaps due to an excessive salience of LICs when only one alternative is proposed, or because the domestic alternative ---healthcare and education--- was not the most desired. % TODO?no shorten by taking out this sentence? 
...
% If global spending increases by less than the proportionate amount for the number of global items, it could indicate that people view global items as a single entity while, on the contrary, some domestic issues were missing from the binary version. % TODO? shorten by taking out this sentence? 

To test whether the results in \cite{fabre_majority_2025} provide an accurate picture of the % relative
...
\subsection{Acceptance of %International 
...
\subsubsection{International Climate Scheme}\label{subsec:ics}
...
\paragraph{Presentation of the Schemes.} ``Cap and dividend'' is a reference climate policy \citep{grubb_greenhouse_1990,bertram_tradeable_1992,baer_equity_2000,barnes_creating_2008,blanchard_major_2021}, % ,young-brun_within-country_2025 ? ,jamieson_climate_2001,rajan_global_2021 3554_economists_economists_2019
...
is accepted by 68\% of respondents (ranging from 56\% in Poland to 88\% in Saudi Arabia).\footnote{The acceptance of the NCS is higher than the support for a \textit{tax}-and-dividend policy found in other surveys \citep{douenne_yellow_2022,mildenberger_limited_2022}, 12~p.p. higher than in \cite{dechezlepretre_fighting_2025}. Indeed, most people prefer emissions trading schemes to carbon taxes \citep{funke_prices_2025}, and support drops (before recovering) in specific contexts, such as the Yellow Vests movement.} 
...
%only among those opposing the GCS is there a majority perceiving majority opposition to the GCS; the median perception of support in one's own country is 20~p.p. higher among those who support the GCS (50\% versus 30\%). 
...
% most respondents who oppose the GCS believe that the policy does not garner majority support, 
% Interestingly, support is strongly correlated with the perception of support: support for the GCS reaches 72\% among the 39\% of respondents who believe that a majority in their country supports it, compared to 44\% among those who do not. Reciprocally, the median perception of support in one's own country is 20~p.p. higher among those who support the GCS (50\% versus 30\%).
...
As expected, the wider the coverage, the higher the acceptance. However, this effect is relatively small, as acceptance is only 3.8~p.p. higher in the \textit{High} variant (at 68\%) compared to the \textit{Low} variant (65\%). Interestingly, acceptance among Europeans significantly increases when China is added to the coalition, but does not rise further 
when other HICs are also added. Conversely, for Americans and Japanese, the participation of the EU or China yields similar levels of acceptance, and only the combined participation of China, the EU, and other HICs significantly increases acceptance. 
...
\subsubsection{Wealth Tax Funding LICs}\label{subsec:wealth_tax}
...
Here again, acceptance increases with the country coverage, but the effect is small. The middle-ground \textit{HIC} variant garners 70\% acceptance (from 58\% in Switzerland to 81\% in Saudi Arabia). Compared to \textit{HIC}, acceptance is 4.8~p.p. higher with \textit{Global} coverage, while it is only 1.4~p.p. and non-significantly lower with \textit{International} coverage (Figure \ref{fig:wealth_tax}). 

% # 5b. wealth tax by coverage --- moved to Appendix for Science Advances initial submission (see app.tex, label fig:wealth_tax)
...
\subsection{Sincerity of Support for Global Redistribution\label{sec:sincerity}}
...
\subsubsection{Conjoint Analysis}\label{subsec:conjoint}
...
More specifically, following \cite{hainmueller_causal_2014}, Figure \ref{fig:conjoint} and Table \ref{tab:conjoint} present the results %estimates of coefficients $\beta_1$ and $\beta_2$ 
of the following regression, estimated by simple OLS with standard errors clustered by respondent:\footnote{More specifically, I estimate the average marginal component effect (AMCE), which is the change in the probability that a program is chosen if a policy is present rather than not, averaged across all possible values in the other policy domains. \cite{hainmueller_causal_2014} show that the linear model is an unbiased estimator of the AMCE under uniform and independent randomization of attributes. %, even though the dependent variable is binary. 
I verify that the results are similar when using a conditional logit: the average marginal effects of \textit{Int'l tax} and \textit{Cut aid} are 6~p.p. and $-$3~p.p., respectively.}
...
\end{subfigure}
\end{figure}
% TODO? include controls for consistency with warm glow? => no, because there is no correlation between controls and program_preferred (contrary to policy_support), so including them would not improve precision

Both policies significantly affect program choice: the internationally redistributive millionaire tax increases the likelihood that a program is preferred by 5~p.p., while cutting development aid decreases it by 3~p.p. At the country level, the effects are generally non-significant due to lack of power, but when significant, they are of the same sign as the global effect. % (except for \textit{cut aid} in Switzerland). 
On average, the effects of the tax are of similar size to the effects of other policies,\footnote{%Overall, the average absolute effect size (weighted by countries' population sizes) is just 4\% lower for the tax than for all policies.   %6~p.p., 
% Comparing absolute values of coefficients from different regressions is a statistical challenge. As an approximation, I regress the absolute effect size of any policy on a constant and a dummy for either the tax or aid policy, weighting by countries' population sizes. 
A simple permutation test shows that the effects of \textit{int'l tax} is not significantly lower than the average absolute effect size (it is just 4\% lower, $p$=.48) but the effects of \textit{cut aid} is significantly lower ($p$=.01).
% and our effects of interest are not significantly lower than this average (at the 15\% threshold).
} % \footnote{Compared to the average absolute effect size (weighted by countries' population sizes), our effects of interest are 31\% lower on (weighted) average, but the difference is not statistically significant.} %
suggesting that certain global redistribution proposals may be as vote-determining as policies prominent in the national debate. %This conjoint analysis also confirms that stated support for global redistribution is not driven by experimenter demand or exaggerated by social desirability bias, because respondents did not know the survey topic when they answered it.
...
\subsubsection{Testing Warm Glow}\label{subsec:warm_glow}
...
Lower support for the GCS in the treated group would be evidence of warm glow, or moral licensing, as it would indicate that the support derives (at least partially) from moral satisfaction at having recently supported a just cause. 
% Other reasons for Donation reason: self-justifying the donation; that the donation has more value if there is a GCS (otherwise individual donation is pointless)
...
On the contrary, support for the GCS is 3~p.p. higher in the \textit{Donation} branch compared to the control group, and the coefficient is positive in every country, though often not significant (Figure \ref{fig:warm_glow_substitute}, Table~\ref{tab:warm_glow_substitute}). While the reason for this positive effect remains unclear,\footnote{Perhaps the \textit{Donation} question triggers thoughts favorable to the GCS, such as the realization that individual actions like donations are ill-suited to address climate change, so that we need a global policy, even if it is imperfect.} 
...
The treatment was designed to satisfy the exclusion restriction required for the instrumental variables (IV) strategy. The exclusion restriction states that the treatment affects support for global policies only through its impact on beliefs that global redistribution is likely. Table \ref{tab:iv} reports the corresponding regression results. Although the treatment is randomly assigned, the preferred specification includes the sociodemographic variables as controls to improve accuracy.\footnote{See Table~\ref{tab:iv_wo_control} for results without controls.
% In fact, some effects are no longer significant in the specification without controls, with $p$-values for the IV and the direct effect at .11 and .12, respectively (Table~\ref{tab:iv_wo_control}). % AER revision
} 
Informed respondents are 7~p.p. more likely to believe that global redistribution is likely, from a baseline of 33\% in the control group. With an effective F-statistic of 67, this highly significant effect provides a strong first stage for the IV estimation. Assuming that the exclusion restriction holds, the IV is well identified. The local average treatment effect estimated by 2SLS is 18~p.p., indicating that believing global redistribution is likely causally \textit{increases} the share of global policies supported. This estimate is consistent with both the non-causal OLS coefficient of 15~p.p. and the direct effect of the treatment on policy support, estimated at 1~p.p. (see Figure \ref{fig:warm_glow_realism} and Table~\ref{tab:warm_glow_realism}).
...
% TODO:? rewrite more professionally; clarify the logic (causal chain: treatment → belief → acceptance); certainty effects
...
\begin{table}[!htbp] 
  ...
  \makebox[\textwidth][c]{\input{../tables/IV_warm_glow.tex}} % AER _no_star
  \textit{Note: Robust standard errors (HC1) are reported in parentheses. \hfill % \\ %
  $^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01. \\ % AER
...
\subsubsection{Acceptance of Currently Debated Global Policies}\label{subsec:debated}

\paragraph{Plausible Global Policies.}

Figure \ref{fig:solidarity_support_share} reveals the acceptance of plausible global policies (see Figure \ref{fig:solidarity_support_positive} for absolute support). %, i.e. the share of \textit{Somewhat} or \textit{Strong support} among non-\textit{Indifferent} answers. % TODO? (also known as relative support)
...
However, foreign workers do not drive the results, as Saudi citizens exhibit slightly higher support than non-Saudis.\footnote{Therefore, tentative explanations may rather come from Saudi society. While Saudis benefit from a generous welfare state, the Islamic pillar of \textit{Zakat} (almsgiving) might further foster a culture of generosity.} 
As for Italy, it is both the country with the lowest gap in support between left-wing and far-right voters (along with Japan, at 33~p.p.) and the country with the highest support among left-wing voters.\footnote{While the former point may be linked to the vision of Italy's far-right leader of an Africa-Italy partnership (trading off foreign aid with cooperation in fighting immigration), the Italian population might also be influenced by the Vatican's messages in favor of global solidarity.} 
...
I test the preferred amount for the NCQG in grant-equivalent terms, using two random variants. Both variants inform respondents of the current situation and the agreed goal, expressing amounts in both absolute terms and as a proportion of developed countries' GDP. The \textit{Short} variant uses qualitative, textual responses, and features a middle category of \$100~billion (namely, ``Meet the newly agreed goal by tripling grants and loans (\$100 billion in grants, or 0.15\% of GDP).''). The \textit{Full} variant provides more detailed explanations in the question text and then uses numerical answers, with a midpoint of \$300~billion. 

In both variants, the median preferred NCQG is \$100~billion in grants, with 19\% of respondents choosing an amount of \$600~billion or larger (Figures~\ref{fig:ncqg}-\ref{fig:ncqg_full}). 
...
\subsubsection{Support for Radical Proposals, Political Action, and Broad Values}\label{subsec:radical}
...
% of the policy 
Both variants are accepted by a majority in every case except Switzerland for the top~3\% variant (in which case 18\% of Swiss people would be affected). 
Overall, the tax garners %relative majority support
majority acceptance 
...
% TODO compare this to Cappelen_sostad.dta, variables Protest, SocialMedia, VotePref
In a multiple-choice question (censored in Russia), 29\% report they could participate in the movement by either attending a demonstration (19\%), going on strike (7\%), or donating \$100 to a strike fund (10\%). This share rises to 68\% in favor of the movement when including the 52\% of respondents who ``could sign a petition and spread ideas'' (Figure \ref{fig:global_movement}). 
% Taken at face value, these results would mean that a successful global solidarity movement could collect up to \$10~billion and organize some of the largest demonstrations in history, matching Earth Day mobilizations.\footnote{On April 22, 1970, 20 million Americans (10\% of the population) demonstrated for the environment. Since then, Earth Day events regularly mobilize hundreds of millions of people worldwide.} % TODO cut for AER revision
Interestingly, 52\% of the 584 millionaires\footnote{In the weighted subsample of millionaires, 60\% are Americans and 26\% Europeans.} who answered the survey would be in favor of such a movement.
...
for its advocates. % TODO add for AER revision
...
% # 10. group_defended --- moved to Appendix for Science Advances initial submission (see app.tex, label fig:group_defended)

\subsubsection{Preferred Channels for Transferring Resources to LICs}\label{subsec:transfer_how}

Asked to evaluate ways of transferring resources to reduce poverty in LICs on a 4-point Likert scale, the most preferred option in every country is ``Cash transfers to parents (child allowances), to the disabled and to the elderly'', with 16\% selecting it as the \textit{Best way} overall, and 49\% as a \textit{Right way} or \textit{Best way} (Figures \ref{fig:transfer_how}, \ref{fig:transfer_how_above_one}-\ref{fig:transfer_how_negative}).\footnote{Admittedly, it would be more relevant to ask this question to the population of LICs. \cite{carson_publics_2025} survey seven Global South countries and find that 
people favor using carbon tax revenues for public services rather than cash
...
% # 11. transfer_how --- moved to Appendix for Science Advances initial submission (see app.tex, label fig:transfer_how)

% TODO Majority support => say whether there is a test

\subsubsection{Custom Global Income Redistribution}\label{subsec:custom_redistr}
...
The interactive question is available at [link withheld for double-blind review], an explainer video at [link withheld for double-blind review], and the algorithm translating slider positions into a redistribution is described in Appendix \ref{app:algo}. 
...
The average (resp. median) preferred redistribution would reduce the world Gini index from .67 to .59 (resp. .58), the D9/D1 ratio from 42 to 11 (resp. 10), the top~10\% share from 52\% to 47\%, and they would increase the bottom~50\% share from 8\% to 13\%.
...
In the context of a global redistribution, it would be interesting to test whether these results would replicate and whether a majority would accept the average or median redistribution described above. 
%While one cannot be sure that these results would replicate in the context of a global redistribution, they suggest that a majority might accept the average or median redistribution described above. % revision: changed after EJ submission

\section{Discussion} % Summary, conclusion

Applying the theory of optimal taxation, \cite{kopczuk_limitations_2005} show that the level of U.S. foreign aid could only be rationalized if the U.S. government placed a value 2,000 times higher on the welfare of a American than on that of a foreigner (although this ratio should be reduced by the proportion of foreign aid transfers diverted or wasted). 
...
The low salience of global inequality may manifest as a lack of popular mobilization, resulting in it being a low priority for policymakers. Combined with the necessary trade-off between global redistribution and fellow citizens' purchasing power, policymakers may prioritize the latter ---which is the primary concern of voters--- to the point of ignoring universalist attitudes. 
...
Indeed, support for global policies is partly caused by the belief that they are possible, as our information experiment demonstrated. Therefore, the organization of the % solidarity confined at the national level 
...
suggest some untapped potential for global solidarity. In light of these findings, it is unlikely that the public would resist global redistribution policies. This is especially true for balance sheet operations with expansionary impacts and indirect costs, such as debt restructuring, liquidity provision, %currency swaps, 
...
% absence of global polity; trade-off purchasing power / global redistr; policymakers choose to go from lower priority to nothing. Given results, it seems very unlikely that plausible global redistr would be unpopular/resisted by the public: why not Bridgetown? debt relief? BEPS or ad tax? billionaire tax (reinvesting MNE profits into global public investments, Glennie - education among the highest rate of return)
...
\paragraph{Samples.}
I conducted an original survey of 12,001 respondents representative of the adult population in eleven high-income countries (see Figure \ref{fig:country_coverage}). The countries were chosen to span the diversity of high-income countries and the sample sizes to be commensurate with each country's population size.\footnote{The sample sizes are as follows: U.S.: 3,000; Japan: 2,000; Russia: 1,001; Saudi Arabia: 1,000; Europe: 5,000, split in proportion to the countries' adult population sizes (except for Switzerland), i.e. France: 798; Germany: 1,048; Italy: 756; Spain: 603; UK: 826; Poland: 500; Switzerland: 469. The maximum margins of error (at the 5\% threshold) for country samples range from $\pm$1.8 p.p. in the U.S. to $\pm$4.5 p.p. in Switzerland, with an intermediate value of $\pm$3.1 p.p. in Saudi Arabia. } 
...
% TODO! create a README https://social-science-data-editors.github.io/template_README/
% TODO!! AER Romans for Section followed by capital letters for subsections; aea.bst (three authors max); equation number on the left; remove asterisks from table.
...
gender, age (5 brackets), income (4), diploma (3), region (2 to 5), and urbanicity (2 to 3). The samples closely match the actual population frequencies along these dimensions, except for Russia and Saudi Arabia, where individuals without a high school diploma are somewhat underrepresented, as well as low-income individuals in Russia and non-Saudis in Saudi Arabia (see Tables \ref{tab:representativeness_0}-\ref{tab:representativeness_3} in Appendix \ref{app:representativeness}). All results are reweighted to be fully representative of the population along the quotas, with weights trimmed between 0.25 and 4. Results aggregated at the global or European levels weigh each country in proportion to its adult population size. Descriptive results on a random branch use weights that are recomputed within that subsample.

% TODO ajouter p-values throughout
Sociodemographic variables explain 10\% to 15\% of the variance in the main attitudinal outcomes, and this figure drops to 5\% after accounting for country and vote (Figure \ref{fig:lmg}). In other words, although variables such as age and diploma are significantly correlated with attitudes (see Tables \ref{tab:determinant}-\ref{tab:determinants_custom_redistr}), differences in average acceptance of a policy between (say) age groups rarely exceed a dozen percentage points. In contrast, our measure of universalism is a stronger predictor than any sociodemographic variable. % TODO? move to subsec:debated ?

% Appendix \ref{app:pol} shows how the main attitudinal outcomes vary by political leaning. 
While support for the main attitudinal outcomes 
is highest among left-wing voters and lowest among far-right voters, non-voters exhibit attitudes close to the center of the political spectrum (Appendix \ref{app:pol}). Besides, attitudes are much less polarized in Japan compared to Europe and the U.S. 
% Figures \ref{fig:vote_pnr_out}-\ref{fig:vote_representativeness} show % revision
Figure \ref{fig:vote_pnr_out} shows 
how the weighted samples compare to actual voting results in the most recent election. 
...
The questionnaire was carefully worded in a neutral and informative way;\footnote{At the end of the survey, 70\% of the respondents find the survey politically unbiased (Figure \ref{fig:survey_biased}). The most common comment left by respondents in the feedback field is that the survey was ``interesting''; very few criticize it (Figure~\ref{fig:comment_field}).} tested on random people in public spaces to ensure correct comprehension; translated by professional translators, with figures converted into national currencies; and double-checked by native speakers.

Of all respondents who %started the questionnaire, 23\% respondents 
were allowed to continue (as their quotas were not full), %. Among them, 
17\% dropped out (including 10\% who dropped out after the socio-demographic questions). The final sample is obtained after excluding 16\% of respondents from the extended sample for suspicion of low quality: 9\% for failing an attention test and 13\% for completing the questionnaire in less than 6 minutes\footnote{6 minutes corresponds to 30\% of the expected median duration of 20 min. In Russia, the cutoff is 200 seconds, or 30\% of the expected median duration of 11 minutes.} (including 5\% for both reasons). 
I check for differential attrition and finds no correlation of treatment arms with attrition (Appendix \ref{app:attrition}). I also show that the main results replicate in the extended sample (Appendix \ref{app:extended}). 
% Appendix \ref{app:attrition} checks for differential attrition and finds no correlation of treatment arms with attrition. Appendix \ref{app:extended} shows that the main results replicate in the extended sample. 

The order of question items is randomized whenever possible. 
% Appendix \ref{app:order} examines the effect of item order on answers. 
Item order generally has a significant but small effect on answers (2 to 14~p.p.), as shown in Appendix \ref{app:order}. The size of this effect helps identify questions for which opinions are strongly held (e.g. a preference for a sustainable scenario over the status quo) versus weakly held (e.g. the preferred amount of climate finance). % crystallized, well-formed, consolidated, stable, ambivalent

% Appendix \ref{app:comparison} compares the responses to two attitudinal questions borrowed from other surveys. The overall averages differ by 2 to 4~p.p. and the cross-country correlations are high: .70 \citep{global_nation_global_2023} to .86 \citep{cappelen_majority_2025}. % TODO AER revision
...
The project has been approved by the authors' institutional review board (approval reference withheld for double-blind review) and
preregistered in the Open Science Foundation registry (link withheld for double-blind review). The study did not deviate from the registration; the questionnaires and hypotheses tests used are the ones specified \textit{ex ante}. All data, code, and figures from the paper are available in a public repository (link withheld for double-blind review).
...
% \begin{bibunit}[aea] %  AER
\begin{bibunit}[unsrtnat] % Science Advances: numbered, in order of citation
% \begin{bibunit}[chicago] % APSR

\clearpage % AER
% \begin{small}
% Double-blind review: funding, ethics-approval reference, acknowledgements, and competing-interest statement are withheld to preserve author anonymity. The original text is retained (commented / inside a comment environment) for the camera-ready version.
\section*{\normalsize Acknowledgements}
\noindent [Funding sources, institutional review board approval, and acknowledgements are omitted here to preserve author anonymity for double-blind review.]
\iffalse % Original acknowledgements hidden for double-blind review; restore (and delete this \iffalse...\fi) for the camera-ready version.
I received funding from the Agence Nationale de la Recherche for this research (ANR-24-CE03-7110). %The randomized controlled trials were registered in the American Economic Association's registry for randomized controlled trials under ID \href{https://www.socialscienceregistry.org/trials/17248}{AEARCTR-0017248}. %This RCT was registered as \href{https://www.socialscienceregistry.org/trials/17248}{AEARCTR-0017248}. 
The project has been approved by the CIRED institutional review board (IRB-CIRED-2025-2) and %is approved by IRB at Harvard University (IRB21-0137), and 
preregistered in the Open Science Foundation registry (\href{https://osf.io/7mzn4}{osf.io/7mzn4}). 
I am grateful to Alina Timoshkina and Gerhard Toews for helping me launch the survey in Russia. I thank researchers who kindly gave me access to their data: Félix Bajard for wealth distribution and Amory Gethin for income distribution. %, Gerhard Toews for Russian census. 
...
\fi % end of acknowledgements hidden for double-blind review
...
\appendix % NCCcomment
% \pagenumbering{arabic} % APSR
\clearpage
% \begin{center}
%     {\LARGE \bfseries Online Appendix}
% \end{center}
% \renewcommand{\thesection}{\Alph{section}} % AER
...
% ==== Figures moved out of the main text for the Science Advances initial submission ====
\section{Extra Main Figures}\label{app:main_text_figures}

\begin{figure}[h!]
    \caption[Country coverage]{Country coverage of the survey. \hfill (Back~to~Section~\ref{sec:data}.)
...
\begin{figure}[h!]
  \caption[Word cloud of open-ended field, per variant]{Most common concepts in open-ended fields. (Questions~\ref{q:concerns_field}-\ref{q:issue_field}) \hfill (Back~to~Section~\ref{subsec:considerations}.)} \label{fig:wordcloud}
...
\begin{figure}[h!]
    \caption[Support for an international wealth tax depending on country coverage]{Percentage of support for an international wealth tax with 30\% of revenue funding LICs, depending on the country coverage (\textit{Yes}/\textit{No} question). (Questions~\ref{q:global_tax_support}-\ref{q:intl_tax_support}). \hfill (Back~to~Section~\ref{subsec:wealth_tax}.)
...
\begin{figure}[h!]
    \caption[\textit{Right} or \textit{Best way} to transfer resources to LICs (global average)]{``How do you evaluate each of these channels to transfer resources to reduce poverty in LICs?''\\ Percentage of \textit{Right} or \textit{Best way} (other options: \textit{Wrong} or \textit{Acceptable way}). (Question~\ref{q:transfer_how}). \hfill (Back~to~Section~\ref{subsec:transfer_how}.)
...
\end{figure}
% ==== End of figures moved out of the main text ====
\clearpage
