...
I also construct another latent variable of support, using a full-information item factor analysis, based on multidimensional item response theory (MIRT). Contrary to the EFA, this method does not assume that levels in a Likert-scale are separate by one unit; instead, it estimates the parameters to encode Likert-scale that best account for the latent support. Both latent variables share close average values in each subsamples (by country or political leaning). Therefore, even though MIRT is a more accurate method, I use the EFA, whose results are more transparent to describe. \hfill (Back~to~Section~\ref{par:ranking}.)
