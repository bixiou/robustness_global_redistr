...
\begin{figure}[h!]
    ...
    \makebox[\textwidth][c]{\includegraphics[width=\textwidth]{../figures/country_comparison/ncs_gcs_ics_all_control_mean.pdf}} 
\end{figure} 
...
\paragraph{Plausible global policies.} Table \ref{tab:plausible_policies} provides references showing that the ``plausible global policies'' I test (Section~\ref{subsec:debated}) are (similar to proposals) debated in international negotiations.
...
The spreadsheet at \href{https://github.com/bixiou/robustness_global_redistr/raw/main/questionnaire/sources.xlsx}{github.com/bixiou/robustness\_global\_redistr/raw/main/questionnaire/sources.xlsx} contains all such features as well as their sources. % TODO? incl. quotas, etc.
...
% Party leaning
% Policy party
% Wealth tax revenue (2% > 5M)
% NCS/GCS computations
% Emissions ICS
% Info global policies
% NCQG sources
% Top tax revenue + poverty gap
% Custom redistr: global distr
...
While the price increases overtime, emissions decrease so that the increase in carbon-intensive expenditures is estimated to remain at a roughly constant level in the next thirty years, before going down as net-zero approaches. In other words, the cost of climate schemes provided to the respondents reflect the direct monetary costs expected from a carbon price aligned with the Paris Agreement.  

The average increase in expenditures due to the carbon price corresponds to the equal cash transfer each adult would receive in the National Climate Scheme (Question~\ref{q:ncs_support}). Relative to the country's GDP per capita, this translates into the price increase reported in the Global Climate Scheme (GCS). To compute the net cost of the GCS (Question~\ref{q:gcs_support}), I subtract the equal cash transfer from the increase in expenditures. 
In Russia, Saudia Arabia, and the U.S., global country coverage is assumed, resulting in a global basic income of \$36 per month. 
This amount is somewhat lower than the basic income of \$47 corresponding to the world average emissions multiplied by the carbon price times, which would be paid in case of a strictly equal per adult allocation of carbon price revenue. Indeed, the GCS departs from the egalatarian allocation to accommodate special needs of middle-income countries \citep{fabre_plan_2024}. % TODO: conservative, China effect
In European countries and Japan, the cash transfer is even lower, at \$22 per month, since it is computed under a \textit{high} country coverage (cf. Figure \ref{fig:ics_maps}) that excludes countries with the largest emissions per capita. 
