...
\mode<presentation>
{
  \usetheme{Boadilla} % Boadilla / metropolis (nice orange one)
  % \setbeamertemplate{frame footer}{My custom footer} % for metropolis at least
  % \begin{itemize}[<+- | alert@+>] % display and emphasize items one by one (for metropolis at least). To \alert{add orange color}.
  % References (at least for metropolis):
  % \cite{example1992} % in any frame
